          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div @click="feeModel.status === 'active' && (isServiceActive = !isServiceActive)" class="bg-slate-50 dark:bg-slate-800/40 rounded-[28px] p-3 flex items-center justify-between border border-slate-100 dark:border-white/5 cursor-pointer">
              <div class="flex items-center gap-2">
                ...
                <div>
                  <span class="block text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Xizmat ({{ feeModel.percentage }}%)</span>
                  <p class="text-[13px] font-black dark:text-white">{{ isServiceActive ? calculateServiceFee.toLocaleString() : "0" }}</p>
                </div>
              </div>
              <div :class="['w-11 h-6 rounded-full relative transition-all duration-300', isServiceActive ? 'bg-indigo-600' : 'bg-slate-300']">
                <div :class="['w-4 h-4 bg-white rounded-full absolute top-1 transition-all', isServiceActive ? 'left-6' : 'left-1']"></div>
              </div>
            </div>
        ...
        <div class="w-full space-y-2">
          ...
          <div class="flex justify-between items-center bg-indigo-50/50 dark:bg-indigo-500/5 p-4 rounded-[24px]">
            <div>
              ...
              <span class="text-xl font-black text-slate-900 dark:text-white tracking-tighter">{{ finalTotal.toLocaleString() }} <small class="text-[10px] text-indigo-600">uzs</small></span>
            </div>
