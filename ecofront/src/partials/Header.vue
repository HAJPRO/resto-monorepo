      ...
      <div class="flex items-center justify-between w-full h-20 px-4 py-0">
        ...
        <div class="flex items-center space-x-2.5">
          ...
          <button 
            ...
            class="flex items-center justify-center w-10 h-10 border border-white/10 rounded-xl bg-white/5 text-white hover:bg-rose-500/20 hover:text-rose-400 active:scale-90 transition-all group"
            title="Tizimni bloklash"
          >
            <ion-icon :icon="lockClosedOutline" class="text-lg group-hover:scale-110 transition-transform" />
          </button>
          ...
          >
            <ion-icon :icon="isDark ? sunnyOutline : moonOutline" class="text-lg" />
          </button>
...
import { book, notificationsOutline, moonOutline, sunnyOutline,lockClosedOutline } from 'ionicons/icons';

const isDark = ref(false);
import { LockStore } from "../stores/index.store"; // Store manzili

const lockStore = LockStore(); // Storeni chaqiramiz
